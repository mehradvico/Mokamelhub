    ...
    public class ProductService : IProductService
    {
        ...
        private IQueryable<Product> BaseSaerch(ProductInputDto searchDto)
        {
            var query = _context.Products
                ...
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true))
                    .ThenInclude(pi => pi.VarietyItem).ThenInclude(s => s.Variety)
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true))
                    .ThenInclude(pi => pi.VarietyItem2).ThenInclude(s => s.Variety)
                .Include(s => s.Brand)
                .Include(s => s.Status)
                .Include(s => s.Type)
                .Include(s => s.Picture)
                .Where(s => !s.Deleted).AsQueryable();
        ...
        public async Task<BaseResultDto<ProductVDto>> FindAsyncVDto(long id, bool visit = true)
        {
            var item = await _context.Products
                .AsNoTrackingWithIdentityResolution().Include(s => s.Category).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent)
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true)).ThenInclude(pi => pi.VarietyItem).ThenInclude(s => s.Variety)
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true)).ThenInclude(pi => pi.VarietyItem2).ThenInclude(s => s.Variety)
                .Include(s => s.Brand).Include(s => s.Status).Include(s => s.Type).Include(s => s.Picture).SingleOrDefaultAsync(s => s.Id == id && s.Active && s.Deleted != true);
            if (item != null)
            {
                if (visit)
                {
        ...
        public async Task<BaseResultDto<ProductVDto>> FindAsyncVDto(string label, bool visit = true)
        {
            var item = await _context.Products
                .AsNoTrackingWithIdentityResolution().Include(s => s.Category).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent).ThenInclude(s => s.Parent)
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true)).ThenInclude(pi => pi.VarietyItem).ThenInclude(s => s.Variety)
                .Include(s => s.ProductItems.Where(pi => pi.Deleted == false && pi.Active == true && pi.SystemActive == true)).ThenInclude(pi => pi.VarietyItem2).ThenInclude(s => s.Variety)
                .Include(s => s.Brand).Include(s => s.Status).Include(s => s.Type).Include(s => s.Picture).SingleOrDefaultAsync(s => s.ProductLabel == label && s.Active && s.Deleted != true); if (item != null)
            {
                if (visit)
                {
                    item.VisitCount++;
